...
public class Settings {
			...
			public void handleHttpResponse(Net.HttpResponse httpResponse) {
				...
				if (scores.length > 0)
					for (int i = 0; i < scores.length; i++) {
						...
						if (i == 0)
							leaderboardItems[i].setText(String.valueOf(Integer.valueOf(score[score.length - 1]) + 1)
									+ ")" + score[0] + ": " + score[1]);
						else
							leaderboardItems[i] = new Label(String.valueOf(Integer.valueOf(score[score.length - 1]) + 1)
									+ ")" + score[0] + ": " + score[1], Assets.skin);
					}
			}
	...
	public static void sendScore(int score) {
		...
		request.setUrl("http://dreamlo.com/lb/PLfBGtHgG02wU0lSzVNrPAG0uQf3J3-UGzK1i7mXmmxA/add/" + "SpaceGladiator"
				+ "/" + score);

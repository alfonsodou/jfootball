package org.javahispano.jfootball.core.engine.util;

import java.io.Serializable;

/**
 * Class representing horizontal spatial coordinates Clase que representa las
 * coordenadas espaciales horizontales
 */
public final class Position implements Serializable {
	...
	/**
	 * Mueve en un angle y un radio, limitando solo a avanzar no mas de un radio
	 * maximo
	 */
	public Position moveAngle(double angle, double ratio, double maxRatio) {
		return new Position(x + Math.cos(angle) * Math.min(ratio, maxRatio),
				y + Math.sin(angle) * Math.min(ratio, maxRatio));
	}
	...
	/**
	 * Returns the index of the nearest position to this position, excluding a
	 * list
	 */
	/**
	 * Obtiene el indice de la posicion mas cercana a esta posicion, excluyendo
	 * una lista
	 */
	 ...
	 */
	/**
	 * Obtiene un arreglo ordenado de los indices mas cercanos a los mas
	 * lejanos, excluyendo una lista
	 */
	...
	/**
	 * Define a norm to compare distance, the calculation is fatster than use
	 * distance methods
	 */
	/**
	 * Define una norma para comparar distancias, es mas rapido el calculo que
	 * usar el metodos distancia
	 */
	public double norm() {
		return x * x + y * y;
	}

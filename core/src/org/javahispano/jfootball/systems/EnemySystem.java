...
public class EnemySystem extends EntitySystem implements EntityListener {
	...
	ComponentMapper<EnemyComponent> cm = ComponentMapper.getFor(EnemyComponent.class);
	...
	public void update(float delta) {
		...
		for (int i = 0; i < entities.size(); i++) {
			...
			cm.get(e).walkDirection.scl(10f * delta); // TODO make this change
														// on difficulty
	...
	private void spawnEnemy(int randomSpawnIndex) {
		engine.addEntity(EntityFactory.createEnemy(gameWorld.bulletSystem, xSpawns[randomSpawnIndex], 33,
				zSpawns[randomSpawnIndex]));
	}
